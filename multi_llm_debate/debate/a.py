...
class AgentsEnsemble:
    ...
    def _respond_batch(
        ...
        """Generate batch responses to a single prompt for a list of agents.

        Args:
            agent_info (List[AgentInfo]): The list of agent information.
        ...
        responses = []
        start_time = time.time()
        batch_size_local = len(self.agents)
        prompts = [prompt] * batch_size_local

        logger.debug(
            f"{len(self.agents)} agents starting batch request with "
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                api_start = time.time()
                logger.info(
                    f"Agents sending batch request "
                    f"with {batch_size_local} identical prompts"
                )
                try:
                    raw_responses = asyncio.run(
                        call_model_batch(
                            model_name=self.agents[0].model,
                            base_url=self.agents[0].base_url,
                ...
                for i, raw_response in enumerate(raw_responses):
                    processed_response = self._process_response(
                        self.agents[i], raw_response
                    )
                    processed_responses.append(processed_response)

                total_time = time.time() - start_time
                logger.info(
                    f"Batch completed in {total_time:.2f}s with {len(processed_responses)} responses"
                )
        ...
        if use_parallel:
            ...
            for i, agent_info in enumerate(
                ...
                if self.job_delay > 0 and i < len(self.agents) - 1:
                    logger.debug(f"Waiting {self.job_delay}s before next agents")
        ...
        if use_parallel and len(unique_models) > 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for model, agents_group in model_groups.items():
                    ...
                    model_futures[future] = agents_group
            ...
            try:
                agent_results_list: List[List[Dict[str, Any]]] = []
