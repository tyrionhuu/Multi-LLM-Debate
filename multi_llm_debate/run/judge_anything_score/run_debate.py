...
from .prompts import (
    build_judge_anything_score_round_n_prompt,
    build_judge_anything_score_round_zero_prompt,
)
from .utils import extract_1_5_answer, image_path_to_bytes

# Fix the setup_logging call by removing the level parameter
logger = logging.getLogger(__name__)


def process_judge_anything_score_dataset(
    dataframe: pd.DataFrame,
    max_rounds: int = 10,
    base_dir: Path = Path("data") / "judge_anything_score",
    ...
    """
    required_columns = ["question", "response", "id", "image_path"]

    return process_debate_dataset(
        dataframe=dataframe,
        process_entry_fn=process_judge_anything_score_entry,
        required_columns=required_columns,
        base_dir=base_dir,
        max_rounds=max_rounds,
        model_configs=model_configs,
        overwrite=overwrite,
        task_name="MLLM-Judge Score task",
...
def process_judge_anything_score_entry(
    entry: pd.Series,
    max_rounds: int = 10,
    base_dir: Path = Path("data") / "judge_anything_score",
    model_configs: Optional[List[ModelConfig]] = None,
    ...
    """Process a single entry for MLLM-Judge task.
    ...
    process_single_debate_entry(
        ...
        prompt_builder_fn=lambda prompt_params: PromptBuilder(
            round_zero_fn=build_judge_anything_score_round_zero_prompt,
            round_n_fn=build_judge_anything_score_round_n_prompt,
