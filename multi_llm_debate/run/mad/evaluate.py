...
def evaluate_mad_results(
    results: Dict[str, Any], output_dir: Path, **kwargs
) -> Dict[str, Any]:
    ...
    rounds_completed = [
        d.get("rounds_completed", 0) for d in debates if d.get("success", False)
    ]
    avg_rounds = (
        sum(rounds_completed) / len(rounds_completed) if rounds_completed else 0
    )

    # Analyze answer lengths
    debate_answers = [
        d.get("debate_answer", "") for d in debates if d.get("debate_answer")
    ]
    base_answers = [d.get("base_answer", "") for d in debates if d.get("base_answer")]

    avg_debate_answer_length = (
        sum(len(ans) for ans in debate_answers) / len(debate_answers)
        if debate_answers
        else 0
    )
    avg_base_answer_length = (
        sum(len(ans) for ans in base_answers) / len(base_answers) if base_answers else 0
    )
    ...
    for difficulty, stats in difficulties.items():
        difficulty_success_rates[difficulty] = (
            stats["successful"] / stats["total"] if stats["total"] > 0 else 0
        )
