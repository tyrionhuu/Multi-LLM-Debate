...
from .mad_evaluate import evaluate_all_mllm_judge_pair_mad
from .mad_run_debate import run_mllm_judge_pair_mad_debate
...
def main(
    ...
    task_name: str = "mllm_judge_pair_mad",
    ...
    """Run MAD debate evaluation on MLLM-Judge-pairs dataset with configured models.

    Args:
        dataframe: Input DataFrame containing the MLLM-Judge-pairs data
        task_name: Name of the debate task (default: "mllm_judge_pair_mad")
    ...
    if config_json is not None:
        ...
        if config is None:
            config = Path(f"multi_llm_debate/run/mllm_judge_pair/config_gemini.json")
    ...
    for i, model_configs in enumerate(model_configs_list):
        ...
        try:
            results = run_mllm_judge_pair_mad_debate(
                dataframe=dataframe,
            ...
            if run_evaluation:
                ...
                evaluation_results = evaluate_all_mllm_judge_pair_mad(
                    base_dir=Path(f"data/{task_name}"),
...
if __name__ == "__main__":
    args = Parser(description="Run MLLM-Judge-pairs MAD evaluation").parse_args()

    if args.task_name is None:
        task_name = "mllm_judge_pair_mad"
    else:
