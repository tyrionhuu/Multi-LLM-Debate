...
def analyze_mad_response_for_mllm_judge_pair(
    ...
    Args:
        ...
        correct_answer: The correct answer from MLLM-Judge-pairs dataset
...
def evaluate_mllm_judge_pair_mad_results(
    base_dir: Path,
    original_dataframe: pd.DataFrame,
    model_configs: Optional[List[Dict]] = None,
) -> Dict[str, Any]:
    """Evaluate MAD debate results for MLLM-Judge-pairs dataset.

    Args:
        base_dir: Base directory containing MAD debate results
        original_dataframe: Original MLLM-Judge-pairs DataFrame
    ...
    for result_file in result_files:
        try:
            ...
            analysis = analyze_mad_response_for_mllm_judge_pair(mad_answer, correct_answer)
...
def print_mllm_judge_pair_mad_evaluation_summary(evaluation_results: Dict[str, Any]) -> None:
    """Print a summary of MLLM-Judge-pairs MAD evaluation results.

    Args:
        evaluation_results: Results from evaluate_mllm_judge_pair_mad_results
    """
    print("\nMLLM-Judge-pairs MAD Evaluation Summary")
    print("=" * 50)
...
def evaluate_all_mllm_judge_pair_mad(
    base_dir: Path,
    original_dataframe: pd.DataFrame,
    model_configs: Optional[List[Dict]] = None,
) -> Dict[str, Any]:
    """Evaluate all MAD debate results for MLLM-Judge-pairs dataset.
    ...
    Returns:
        Dict containing evaluation results
    """
    logger.info("Starting MLLM-Judge-pairs MAD evaluation")

    # Run evaluation
    evaluation_results = evaluate_mllm_judge_pair_mad_results(
        base_dir=base_dir,
    ...
    # Print summary
    print_mllm_judge_pair_mad_evaluation_summary(evaluation_results)
