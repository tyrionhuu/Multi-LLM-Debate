...
def process_mllm_judge_pairs_dataset(
    ...
    max_rounds: int = 10,
    base_dir: Path = Path("data") / "mllm_judge_pairs",
    ...
    """
    required_columns = ["question", "response_A", "response_B", "id", "image"]

    return process_debate_dataset(
        dataframe=dataframe,
        process_entry_fn=process_mllm_judge_pairs_entry,
        required_columns=required_columns,
        base_dir=base_dir,
        max_rounds=max_rounds,
        model_configs=model_configs,
        overwrite=overwrite,
        task_name="MLLM-Judge Pairs task",
...
def process_mllm_judge_pairs_entry(
    ...
    process_single_debate_entry(
        ...
        prompt_builder_fn=lambda prompt_params: PromptBuilder(
            round_zero_fn=build_mllm_judge_pairs_round_zero_prompt,
