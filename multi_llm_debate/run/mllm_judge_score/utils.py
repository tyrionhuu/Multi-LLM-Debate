...
def load_mllm_judge_score_dataset(
    ...
    try:
        ...
        df["question"], df["response"], df["score"] = zip(*df["question"].apply(parse_question_field))
        ...
        if sample_size is not None:
            if sample_size > len(df):
                logger.warning(f"Requested sample size {sample_size} exceeds dataset size {len(df)}. Returning full dataset.")
                sample_size = len(df)
