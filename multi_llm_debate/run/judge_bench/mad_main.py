...
from .mad_evaluate import evaluate_all_judge_bench_mad
from .mad_run_debate import run_judge_bench_mad_debate
...
def main(
    ...
    Args:
        dataframe: Input DataFrame containing the JudgeBench data
        task_name: Name of the debate task (default: "judge_bench_mad")
    ...
    if config_json is not None:
        ...
        if config is None:
            config = Path(f"multi_llm_debate/run/judge_bench/config_gemini.json")
    ...
    for i, model_configs in enumerate(model_configs_list):
        ...
        try:
            results = run_judge_bench_mad_debate(
                dataframe=dataframe,
            ...
            if run_evaluation:
                ...
                evaluation_results = evaluate_all_judge_bench_mad(
                    base_dir=Path(f"data/{task_name}"),
