...
def process_prm800k_dataset(
    ...
    """Run the PRM800K task on a DataFrame.
    ...
    """
    required_columns = ["question", "steps", "id", "answer"]

    return process_debate_dataset(
        dataframe=dataframe,
        process_entry_fn=process_single_prm800k_entry,
...
def process_single_prm800k_entry(
    ...
    """Process a single entry for PRM800K evaluation.

    Args:
        entry: Pandas Series containing question, steps, id, answer
        max_rounds: Maximum number of debate rounds
        model_configs: Optional list of model configurations. If None,
                       default configs will be used.
        base_dir: Base directory for output files
    ...
    Returns:
        Dict containing processed entry information
    """
    logger.info(f"Processing PRM800K entry ID: {entry['id']}")
    required_columns = ["question", "steps", "id", "answer"]
    prompt_params = {
    ...
    return process_single_debate_entry(
        ...
        prompt_builder_fn=lambda prompt_params: PromptBuilder(
            round_zero_fn=build_prm800k_round_zero_prompt,
            round_n_fn=build_prm800k_round_n_prompt,
