...
# Build the command
CMD="python -m multi_llm_debate.run.mllm_judge_pair.main --mad --config-json '$CONFIG' --task-name \"mllm_judge_pair_mad\""
...
echo "Running MAD MLLM-Judge-pairs evaluation with vLLM (Gemma)..."
echo "Command: $CMD"

# Run the evaluation
eval $CMD

echo "MAD MLLM-Judge-pairs evaluation with vLLM (Gemma) completed!"
cleanup 0 