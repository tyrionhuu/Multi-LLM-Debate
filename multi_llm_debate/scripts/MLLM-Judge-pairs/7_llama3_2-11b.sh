...
if [[ "$GPU" == *","* ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 16000 --tensor-parallel-size ${NUM_GPUS} --gpu-memory-utilization 0.95 --enforce-eager --max-num-seqs 8 &
else
    # Single GPU mode
    ...
    env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 16000 --gpu-memory-utilization 0.95 --enforce-eager --max-num-seqs 8 &
fi
