...
# Build the command
CMD="python -m multi_llm_debate.run.judge_anything_pair.main --mad --config-json '$CONFIG' --task-name \"judge_anything_pair_mad\""
...
echo "Running MAD JudgeAnything-pair evaluation with vLLM (Gemma)..."
echo "Command: $CMD"

# Run the evaluation
eval $CMD

echo "MAD JudgeAnything-pair evaluation with vLLM (Gemma) completed!"
cleanup 0 