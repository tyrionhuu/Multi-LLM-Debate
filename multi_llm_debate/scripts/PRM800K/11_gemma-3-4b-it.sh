...
if [[ "$GPU" == *","* ]]; then
    ...
    if [[ ${#GPU_ARRAY[@]} -eq 2 ]]; then
        ...
        env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 20000 --tensor-parallel-size 2 &
    else
    ...
    # Single GPU mode
    env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 20000 --gpu-memory-utilization 0.94 &
fi
