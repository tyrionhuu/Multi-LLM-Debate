...
if [[ "$GPU" == *","* ]]; then
    ...
    if [[ ${#GPU_ARRAY[@]} -eq 2 ]]; then
        ...
        CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 16000 --tensor-parallel-size 2 &
    else
    ...
    # Single GPU mode
    CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 16000 &
fi
