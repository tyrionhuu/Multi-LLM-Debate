...
if [[ "$GPU" == *","* ]]; then
    ...
    env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 32000 --tensor-parallel-size ${NUM_GPUS} --gpu-memory-utilization 0.95 &
else
    # Single GPU mode
    env CUDA_VISIBLE_DEVICES=$GPU vllm serve $MODEL_NAME --host 0.0.0.0 --port $PORT --max-model-len 32000 --gpu-memory-utilization 0.95 &
fi
...
CUDA_VISIBLE_DEVICES=all python -m multi_llm_debate.run.judge_anything_score.main \
    --task-name "judge_anything_score" \
    --config-json "$CONFIG" \
    --batch \
    --batch-size 11 \
    --sample-size 800 \
...
CUDA_VISIBLE_DEVICES=all python -m multi_llm_debate.run.judge_anything_score.main \
    --config-json "$CONFIG" \
    --task-name "judge_anything_score_pruning" \
