...
def run_sensitivity_study(
    ...
    for threshold in tqdm(ks_thresholds, desc="Testing KS thresholds"):
        ...
        try:
            ...
            stopped_early = rounds_processed < len(aggregated_df) if max_rounds is None else rounds_processed < max_rounds
            
            # Get final distribution parameters
            final_params = fit_results[-1] if fit_results else None
            
            # Calculate convergence metrics
            convergence_metrics = calculate_convergence_metrics(ks_statistics, threshold)
            ...
            logging.info(f"  Final KS stat: {final_ks_stat:.4f}" if final_ks_stat else "  Final KS stat: None")
...
def calculate_convergence_metrics(ks_statistics: List[Optional[float]], threshold: float) -> Dict:
    """
    ...
    metrics = {
        ...
        'convergence_rate': sum(1 for ks in valid_ks_stats if ks < threshold) / len(valid_ks_stats)
    }
...
def plot_sensitivity_results(results: Dict, output_dir: Path) -> None:
    ...
    if not valid_df.empty:
        ...
        axes[0, 0].plot(valid_df['ks_threshold'], valid_df['rounds_processed'], 'o-', linewidth=2, markersize=8)
        ...
        axes[0, 1].plot(valid_df['ks_threshold'], valid_df['final_ks_statistic'], 's-', linewidth=2, markersize=8)
        ...
        axes[1, 0].plot(valid_df['ks_threshold'], valid_df['convergence_rate'], '^-', linewidth=2, markersize=8)
        ...
        axes[1, 1].plot(valid_df['ks_threshold'], valid_df['mean_ks'], 'd-', linewidth=2, markersize=8)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'ks_threshold_sensitivity_summary.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    representative_thresholds = sorted([t for t in results.keys() if 'error' not in results[t]])[:4]
    ...
    for i, threshold in enumerate(representative_thresholds):
        ...
        if ks_stats:
            ...
            axes[row, col].plot(rounds, ks_stats, 'o-', linewidth=2, markersize=6, label=f'Threshold: {threshold}')
            axes[row, col].axhline(y=threshold, color='r', linestyle='--', alpha=0.7, label=f'Threshold ({threshold})')
            axes[row, col].set_xlabel('Round Number')
            axes[row, col].set_ylabel('KS Statistic')
            axes[row, col].set_title(f'KS Statistics Evolution (Threshold: {threshold})')
            axes[row, col].legend()
            axes[row, col].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(output_dir / 'ks_statistics_evolution.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    parser.add_argument('--debates-path', type=Path, required=True,
                       help='Path to the debates CSV file')
    parser.add_argument('--output-dir', type=Path, default=Path('output/ks_threshold_sensitivity'),
                       help='Output directory for results')
    parser.add_argument('--ks-thresholds', type=float, nargs='+',
                       default=[0.01, 0.02, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20],
                       help='List of KS threshold values to test')
    parser.add_argument('--stability-rounds', type=int, default=2,
                       help='Number of consecutive stable rounds required')
    parser.add_argument('--fitting-method', type=str, default='direct',
                       choices=['direct', 'em'],
    ...
    parser.add_argument('--n-restarts', type=int, default=2,
                       help='Number of restarts for fitting')
