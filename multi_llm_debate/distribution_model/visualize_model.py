...
def plot_mixture_model(
    ...
    ax.plot(x, y_mixture, marker='o', linestyle='-', color=color, 
            alpha=alpha, label='Mixture Model')
    
    # Plot the individual components
    ax.plot(x, [w * y for y in y_comp1], linestyle='--', color=color, 
            alpha=alpha*0.7, label='Component 1')
    ax.plot(x, [(1-w) * y for y in y_comp2], linestyle=':', color=color, 
            alpha=alpha*0.7, label='Component 2')
...
def plot_model_evolution(
    ...
    fig, axes = plt.subplots(
        nrows=2, 
        ncols=math.ceil(len(model_results)/2),
        figsize=(15, 10)
    )
    axes = axes.flatten()
...
def visualize_parameter_trends(
    ...
    # Plot mixture weight
    axes[0].plot(rounds, w_values, marker='o', linestyle='-', label='Mixture Weight (w)')
